#!/bin/bash
# Train a GRPO agent on an AI Safety Gridworlds environment.
#
# Usage:
#   bash grpo_train.sh
#   ENV_NAME=WhiskyGold MODEL_PATH=Qwen/Qwen2.5-14B-Instruct bash grpo_train.sh
#   bash grpo_train.sh env.history_length=4        # extra key=value args go to the trainer
#
...
MODEL_PATH="${MODEL_PATH:-Qwen/Qwen2.5-7B-Instruct}"     # base model
...
EXPERIMENT_NAME="${EXPERIMENT_NAME:-${ENV_NAME}-grpo-$(basename "${MODEL_PATH}")}"
SEED="${SEED:-1}"
